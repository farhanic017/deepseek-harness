...
describe('minimal agent preset', () => {
  ...
  it('sends the exact RL prompt and schemas, then executes the persistent shell and editor', async () => {
    ...
    // win32 runs the fresh-process `pwsh` face of the two-tool composition
    // (the persistent bash PTY cannot run there), so the shell probe is one
    // self-contained command proving the shell reaches the state directory;
    // POSIX keeps the two-call persistent-bash state proof.
    ...
    const bash = isWin32
      ? await scaffold.ctx.tools.execute({
        ...
        name: 'pwsh',
        arguments: {
          command: `Set-Location ${JSON.stringify(stateDir)}; Write-Output \"PERSISTED:$((Get-Location).Path)\"`,
          description: 'prove the win32 shell reaches the persistent-state directory',
        },
    ...
    const text = (result: typeof bash): string => result.content
      ...
      // Windows tool output renders paths with backslashes; both spellings
      // must collapse to the same golden.
      .replaceAll(scaffold.workspaceCwd.split('\\').join('/'), '{{cwd}}')
      .replaceAll('\\', '/')
      .trimEnd()

    // The shell tool name is the platform's face of the same two-tool
    // composition: persistent `bash` on POSIX, fresh-process `pwsh` on win32.
    const toolNames = (requestHeader.tools?.map(tool => tool.name) ?? []).toSorted()
    expect(toolNames).toEqual(isWin32
