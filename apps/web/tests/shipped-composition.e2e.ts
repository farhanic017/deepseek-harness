 ...
 */
/** The shell tool name is the platform face of the same row: `bash` on POSIX, `pwsh` on win32. */
...
// The shell tool sorts by name: `bash` lands right after ask_user_question,
// `pwsh` between list_agents and ralph. Insert at its sorted position so one
// list serves both platforms.
EXPECTED_TOOLS.splice(
  EXPECTED_TOOLS.findIndex(name => name.localeCompare(SHELL_TOOL) > 0),
  0,
  SHELL_TOOL,
)
