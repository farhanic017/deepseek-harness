...
export default defineConfig({
  ...
  // Relative asset base so the built dist also loads over Electron's file://
  // protocol (an absolute `/assets/...` base resolves to the filesystem root
  // there, 404s, and leaves the renderer a white page). HTTP hosts (dsh web)
  // resolve the same relative URLs against the page URL, so both carriers work.
  base: './',
  build: {
    sourcemap: true,
    rollupOptions: {
