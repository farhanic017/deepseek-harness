...
// The renderer serves only same-origin content from the local web server, so
// webSecurity never needs disabling. The dev-mode CSP warning cannot be
// truthfully removed: the client-runner executes every plugin's client half
// through `new Function`, which a real CSP must allow via unsafe-eval — and
// Electron warns about that in dev. These are dev-only diagnostics (packaged
// apps never log them), so suppress them in dev instead of shipping a
// misleading policy.
