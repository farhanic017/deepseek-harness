...
- A standalone packaged build **does** boot on its own: the anchor walk finds
  the bundled `resources/workspace` tree (the materialized `pnpm deploy`
  output), and `linkBundledWorkspace` repairs any junction that does not
  resolve inside the tree before the Loader imports the first plugin. The
  web profile boots and serves the UI without a checkout on disk.
- Packaging **from source** is fragile on Windows: `fs.cpSync({ dereference: true })`
  under Node ≥ 22 expands the pnpm junction cycles (`vendor/cordis` ↔
  `vendor/include`) until it crashes, and robocopy/xcopy either follow the
  junctions into a combinatorial blowup or silently truncate content. The
  reliable production path is to patch a known-good `win-unpacked` in place
  (see `dist/patch-app.cjs`) rather than re-materialize the workspace.
