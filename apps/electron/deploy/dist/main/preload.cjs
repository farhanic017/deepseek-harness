			...
			menu.items.forEach((mi) => {
				if (mi.separator) {
					...
					miBtn.addEventListener("click", () => {
						closeAll();
						electron.ipcRenderer.send("menu:action", mi.action);
					});
...
if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", injectMenuBar);
else injectMenuBar();
const electronAPI = {
	dsh: {
		getPort: () => electron.ipcRenderer.invoke("dsh:get-port"),
		onReady: (callback) => {
			electron.ipcRenderer.on("dsh:ready", (_event, data) => callback(data));
			return () => electron.ipcRenderer.off("dsh:ready", (_event, data) => callback(data));
		}
	},
	dialog: {
	...
	shell: { openExternal: (url) => electron.ipcRenderer.invoke("shell:open-external", url) },
	app: {
		getVersion: () => electron.ipcRenderer.invoke("app:get-version"),
		getPath: (name) => electron.ipcRenderer.invoke("app:get-path", name)
	},
	menu: { action: (action) => electron.ipcRenderer.send("menu:action", action) },
