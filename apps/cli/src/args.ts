...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  const electron = program.command('electron').description('boot the electron profile (alias of --profile electron); the electron app\'s own flags follow')
  electron
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the electron app (see: dsh electron --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
