...
export class WorkspaceRuntime implements IWorkspaces {
  ...
  startSession(workspaceId?: WorkspaceId): void {
    ...
    if (target === undefined) {
      // No workspace — create a blank session without a workspace so chat
      // and the model selector work immediately. The workspace picker stays
      // as an optional row in the hero, not a gate.
      void (this.sessions.create as unknown as (opts: Record<string, never>) => Promise<import('@deepseek-ai/dsh-api-remotes/client').SessionId>)({}).then(
