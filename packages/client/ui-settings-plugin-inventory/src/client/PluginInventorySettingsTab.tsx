...
export interface PluginInventorySettingsTabInjected {
  ...
  /** Toggle a plugin entry's enabled state. Returns the new enabled state. */
  toggle: (entryId: string) => Promise<boolean>
}
...
export function PluginInventorySettingsTab({ list, toggle, t }: PluginInventorySettingsTabProps): ReactNode {
  ...
  const handleToggle = async (entry: PluginInventoryEntry): Promise<void> => {
    try {
      const newEnabled = await toggle(entry.entryId)
      setState((prev) => {
        if (prev.status !== 'ready') return prev
        return {
          ...prev,
          snapshot: {
            entries: prev.snapshot.entries.map(e =>
      ...
      // Toggle failed — retry the full list to re-sync.
      setRequest(v => v + 1)
    }
  }
                  ...
                  >
                    <div className={css.cardRow}>
                      ...
                      <button
                        ...
                        title={t('toggle')}
                        onClick={(e) => { e.stopPropagation(); void handleToggle(entry) }}
                      >
                        <span className={css.toggleTrack} data-on={entry.enabled || undefined}>
