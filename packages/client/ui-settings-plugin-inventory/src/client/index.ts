...
export function apply(ctx: ClientContext): void {
  ...
  const toggle: PluginInventorySettingsTabInjected['toggle'] = async (entryId: string) => {
    const result = await ctx.remote.pluginInventory.toggle(entryId as Parameters<typeof ctx.remote.pluginInventory.toggle>[0])
    if (!result.ok) {
