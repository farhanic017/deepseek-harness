...
export class AgentPresetSeatController {
  ...
  /**
   * The preset the current flow applied to a session, so a later roster load
   * cannot regress the display back to the deployment default while the
   * applied pick is the truth. The sessions list is the durable record of
   * what a session runs, but a list response computed before the switch
   * committed can land late and still carry the session's creation-time
   * preset — deriving the chip from that row alone would snap an applied
   * pick back to Standard mode. Cleared when the flow's session changes.
   */
  private applied: string | undefined

  /** The session the applied pick landed on; a different current means the flow moved on. */
  private appliedTo: SessionId | undefined
  ...
  async load(): Promise<void> {
    try {
      ...
      this.set({
        ...
        // Staged pick first, then the pick this flow already applied, then
        // the composition the current session carries, then the deployment
        // default. The applied term is what keeps a late-landing load from
        // regressing the display after an applied stage was consumed: the
        // chip mounts (and loads) only once the flow's session is current,
        // so the reply can arrive after apply() already composed it, and a
        // session.list computed before the switch committed can still report
        // the creation-time preset.
  ...
  async apply(): Promise<void> {
    ...
    // The applied pick belongs to the flow it landed on: a different (or
    // absent) current session means that flow is no longer in front of the
    // chip, and the next session opens on the deployment default again.
    if (this.appliedTo !== undefined && (session === undefined || session.id !== this.appliedTo)) {
    ...
    try {
      ...
      this.appliedTo = session.id
      this.set({ busy: false, current: this.applied })
      this.onApplied?.(session.id, this.applied)
    } catch (error) {
      this.staged = undefined
