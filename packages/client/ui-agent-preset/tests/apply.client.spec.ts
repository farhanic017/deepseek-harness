...
describe('ui-agent-preset apply', () => {
  ...
  it('keeps an applied pick when a late roster load finds the session row on its creation-time preset', async () => {
    const { ctx, slots } = await bench()
    declareRoot(slots)
    const conversation = declareConversation(slots)
    ctx.provide('conversation', {} as never)
    ...
    // A session.list response computed before the switch committed lands late
    // and still reports the session under its creation-time preset (the apply
    // echo updated the row, this refresh overwrites it back). A subsequent
    // roster load — a chip remount, a settings refresh — must not snap the
  ...
  it('forgets the applied pick once the flow moves to another session', async () => {
    const { ctx, slots } = await bench()
    declareRoot(slots)
    const conversation = declareConversation(slots)
    ctx.provide('conversation', {} as never)
    ...
    // The next session is a different blank one: the chip opens on the
    // deployment default again instead of carrying the previous pick over.
    state.current = 's2'
    state.byId['s2'] = { id: 's2', blank: true, agentPreset: 'standard' }
    sessions.notify()
    await seat.load()
