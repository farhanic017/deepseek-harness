...
describe('ModelSelect reasoning effort', () => {
  ...
  it('filters models by search query in the model pane', () => {
    const directory = createSnapshotStore<ModelDirectoryState>(state({
      groups: [
        {
          id: 'deepseek-official',
        ...
        },
        {
          ...
          name: 'OpenRouter',
          models: [
            { id: 'jamba', name: 'AI21: Jamba Large 1.7' },
            { id: 'nova', name: 'Amazon Nova 2 Lite' },
          ],
        },
      ],
      current: { provider: 'deepseek-official', model: 'deepseek-v4-flash' },
    ...
    expect(screen.getAllByRole('menuitemradio').map(item => item.textContent)).toEqual([
      'DeepSeek-V4-Flash', 'DeepSeek-V4-Pro', 'AI21: Jamba Large 1.7', 'Amazon Nova 2 Lite',
    ])

    // Type a query — only DeepSeek models remain.
    const search = screen.getByPlaceholderText('搜索模型') as HTMLInputElement
    fireEvent.change(search, { target: { value: 'flash' } })
    expect(screen.getAllByRole('menuitemradio').map(item => item.textContent)).toEqual([
      'DeepSeek-V4-Flash',
    ])

    // Clear the query — all models return.
