...
export function ConversationRoot({
  ...
  renderSlot, renderSlotChain, selectWorkspace, createSession, removeWorkspace, t,
  ...
  // Auto-create a blank session so chat + model work without picking a workspace.
  useEffect(() => {
    if (sessionId !== undefined) return
    if (pendingWorkspaceId !== undefined) return
    createSession()
  }, [sessionId, pendingWorkspaceId, createSession])
  ...
  const chipWorkspaceId = pendingWorkspace?.workspaceId ?? sessionWorkspace?.workspaceId
  const heroWorkspaceRow = (
    <div className={css.heroWorkspaceRow}>
      <WorkspaceChip
        ...
        {...(chipWorkspaceId === undefined ? {} : { onRemove: () => { void removeWorkspace(chipWorkspaceId).catch(() => {}) } })}
        t={t}
      />
  ...
  // Workspace is optional — chat and model selector work without a workspace.
  // The chip remains as a quick picker in the hero row, but it no longer
  // blocks the composer. Inert is a prop, not a different tree.
  const inert = false
