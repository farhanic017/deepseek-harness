      ...
      <div className={css.sectionHeader}>
        ...
        <div className={clsx(css.headerActions, wide && searchExpanded && css.headerActionsHidden)}>
          ...
          {wide && workspaces.length > 0 && (
            <Tooltip label="Remove selected workspaces" side="bottom" delayMs={500}>
              <button
                type="button"
                className={css.iconButton}
                aria-label="Remove selected workspaces"
                onClick={() => {
                  const idsToRemove = workspaces.map(w => w.workspaceId)
                  if (idsToRemove.length === 0) return
                  if (!confirm(`Remove ${idsToRemove.length} workspace(s)? This will detach them from the list (files stay on disk).`)) return
                  for (const id of idsToRemove) {
                    deleteWorkspace(id).catch((reason: unknown) => {
                      console.warn('workspace remove failed:', reason)
                    })
                  }
                }}
              >
                <IconTrashOutline16 size={wide ? 16 : 18} />
              </button>
            </Tooltip>
          )}
        </div>
