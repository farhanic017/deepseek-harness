...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  ...
  electron: ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-electron-app'],
}
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    ...
    // junction as a directory and throws EISDIR unless recursive. A concurrent
    // launch may already have removed the stale link between the lstat above
    // and this unlink — the link is then gone, which is the outcome we wanted.
    try {
      unlinkSync(link)
    } catch (error) {
