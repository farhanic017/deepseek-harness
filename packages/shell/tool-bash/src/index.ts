 ...
 * otherwise use the filesystem identity of the session cwd. When no workspace
 * is selected and no explicit workdir is given, fail fast instead of falling
 * back to process.cwd() — scanning the server's repo root is slow and not what
 * the user asked for when they haven't picked a folder.
 */
function resolveWorkdir(
  ...
  if (modelWorkdir === undefined) {
    if (sessionCwd === undefined) {
      throw new Error(
        'No workspace selected — cannot run bash without a workspace. ' +
        'Ask the user to pick a workspace, or provide an explicit workdir if the command does not need one.',
      )
    }
    return sessionCwd
  }
