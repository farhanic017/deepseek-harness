...
export async function pickNativeDirectory(
  ...
  if (platform === 'win32') {
    // In Electron, use the Electron dialog directly — it's more reliable than
    // the koffi worker and doesn't require a separate child process. The koffi
    // path remains as a fallback for non-Electron hosts.
    if (process.versions.electron !== undefined) {
      try {
        // @ts-ignore — electron is an optional peer, only available in Electron
        const electron = await import('electron')
        const result = await (electron.dialog as unknown as { showOpenDialog(options: { properties: string[]; title: string }): Promise<{ canceled: boolean; filePaths: string[] }> }).showOpenDialog({
