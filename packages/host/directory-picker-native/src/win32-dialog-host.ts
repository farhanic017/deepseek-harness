...
export function spawnDialogWorker(data: Win32DialogWorkerData): ReturnType<typeof spawn> {
  // Under Electron, process.execPath is the Electron binary: spawning it with
  // a script argument would launch a second app instance (its own
  // GPU/utility/renderer processes) instead of running the plain-Node worker
  // entry — the dialog never appears and the pick hangs. ELECTRON_RUN_AS_NODE
  // makes the same binary behave as plain Node, which is exactly what the
  // worker.cjs entry expects; a plain-Node host ignores the variable.
  const env = {
    ...process.env,
